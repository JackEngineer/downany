...
当前已实现失败任务到登录状态、代理、下载工具和下载位置的定向入口、添加前下载目录预检和扩展连接状态，并建立可机器判定的 30 条场景矩阵。macOS 工程候选已完成 YouTube 8/10、Bilibili 8/10 和抖音失效链接 1/1，首次安装、v0.3.0 覆盖升级、设置与队列保留、暂停续传、失败恢复、成品解码、防覆盖及候选扩展连接自动化均有包级证据。仍缺 13 条授权登录样本、Windows x64 候选包及全部 Windows 矩阵、五位真实首次用户和双平台人工记录，因此不能标为通过。
