...
## 当前执行：Windows 稳定性升级（2026-08-27）

本轮按已批准的 [分阶段升级规格](superpowers/specs/2026-08-26-downany-v0-3-staged-upgrade-design.md) 和 [Windows 执行范围](superpowers/specs/2026-08-27-downany-windows-execution-scope.md) 逐步推进到 v0.3.0。以下是本地候选进展，不代表已提交、已集成或已公开发布。

- **v0.2.4：成品正确性基线。** 保留已完成的输出校验、解析面板修正和最终文件大小持久化改进，作为下一轮的可追溯源码快照。
- **v0.2.5：队列与恢复稳定性，本地候选已验证。** 已实现可持久化的暂停/继续意图、原子批量操作、分组排序、进度批量更新与旧下载线程隔离；758 项 Python、410 项桌面测试通过。R2 Windows 包完成旧版数据迁移、五轮恢复、真实媒体输出及 Electron 隔离启动验证，详见 [候选验证记录](acceptance/v0.2.5-queue-recovery-stability.md)。未安装、提交或发布。
- **v0.3.0：稳定交付收敛，本地候选完成。** 已统一启动/重连/操作后的队列刷新保护，补齐设置保存竞态、旧格式数据合同和中英文关键路径；独立审查、完整回归、五轮真实旧包恢复、三轮旧格式保留与隔离桌面入队通过。结果见 [v0.3.0 验证记录](acceptance/v0.3.0-stable-baseline.md)，尚未安装、提交或公开发布。
